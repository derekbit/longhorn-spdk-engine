...
func (b *Backup) overlayFragmaps(fragmap []byte) {
	for i := 0; i < len(fragmap); i++ {
		b.fragmap.Map[i] |= fragmap[i]
	}
}

func (b *Backup) getLvolFragmap(lvolName string) ([]byte, error) {
	fragmap, err := b.spdkClient.BdevLvolGetFragmap(b.replica.LvsName, lvolName, 0, b.replica.SpecSize)
	if err != nil {
		return nil, err
	}
	logrus.Infof("fragmap=%+v", fragmap)
	return base64.StdEncoding.DecodeString(fragmap.Fragmap)
}
...
func (b *Backup) constructFragmap(from, to int) error {
	for i := from; i > to; i-- {
		...
		if lvol != nil {
			...
			fragmap, err := b.getLvolFragmap(lvol.Name)
			if err != nil {
				return err
			}
			b.overlayFragmaps(fragmap)
		}
	}
	return nil
}
