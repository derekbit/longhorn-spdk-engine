...
func (s *Server) DiskCreate(ctx context.Context, req *spdkrpc.DiskCreateRequest) (*spdkrpc.Disk, error) {
	...
	if exists {
		...
		s.Unlock()
		return &spdkrpc.Disk{State: string(disk.State)}, nil
	}

	disk = NewDisk(req.DiskName)
	s.diskMap[req.DiskName] = disk
	s.Unlock()

	go func(d *Disk, req *spdkrpc.DiskCreateRequest) {
		_, err := d.DiskCreate(spdkClient, req.DiskName, req.DiskUuid, req.DiskPath, req.DiskDriver, req.BlockSize)
		if err != nil {
			logrus.Errorf("failed to create disk %s(%s): %v", req.DiskName, req.DiskUuid, err)
