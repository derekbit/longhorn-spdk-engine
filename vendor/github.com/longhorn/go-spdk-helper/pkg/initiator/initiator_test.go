...
func TestSetExportDevicePreservesExistingSource(t *testing.T) {
	i := &Initiator{
		dev: &helperutil.LonghornBlockDevice{
			Source: helperutil.BlockDevice{Name: "nvme4n1", Major: 259, Minor: 0},
		},
	}

	i.setExportDevice(helperutil.BlockDevice{Name: "test-vol", Major: 252, Minor: 4})

	if i.dev == nil {
		t.Fatal("expected initiator device to be initialized")
	}
	if i.dev.Source.Name != "nvme4n1" {
		t.Fatalf("expected source device to be preserved, got %q", i.dev.Source.Name)
	}
	if i.dev.Export.Name != "test-vol" {
		t.Fatalf("expected export device name to be updated, got %q", i.dev.Export.Name)
	}
}

func TestSetExportDeviceInitializesDeviceWhenMissing(t *testing.T) {
	i := &Initiator{}
